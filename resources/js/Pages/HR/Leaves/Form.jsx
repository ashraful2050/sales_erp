...
export default function LeaveForm({ leave, leaveTypes, employees }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        isEdit
            ? put(route("hr.leaves.update", leave.id))
            : post(route("hr.leaves.store"));
    };

    return (
        <AppLayout
            title={isEdit ? t("Edit Leave Request") : t("Apply for Leave")}
        >
            <Head
                title={isEdit ? t("Edit Leave Request") : t("Apply for Leave")}
            />
            <PageHeader
                ...
                subtitle={
                    isEdit
                        ? t("Update leave request details")
                        : t("Submit a new leave application")
                }
                actions={
                    <Link
            ...
            <form onSubmit={submit} className="max-w-2xl space-y-6">
                            ...
                            >
                                ...
                                {leaveTypes?.map((lt) => (
                                    <option key={lt.id} value={lt.id}>
                                        {lt.name} ({lt.allowed_days} days/yr)
                                    </option>
                                ))}
                            </Select>
                        </Field>
                ...
                <div className="flex justify-end gap-3 pb-8">
                    ...
                    >
                        <Save size={16} />{" "}
                        {isEdit ? t("Update Leave") : t("Submit Application")}
                    </button>
                </div>
            </form>
        </AppLayout>
