            ...
            <div className="p-6 space-y-6">
                    ...
                    <dl className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <dd className="font-medium">{employee.join_date ?? "—"}</dd>
                            </div>
                        </div>
                        <div>
                            <dt className="text-xs text-gray-500">NID</dt>
                            <dd className="font-medium">{employee.nid ?? "—"}</dd>
                        </div>
                        <div>
                            <dt className="text-xs text-gray-500">Employment Type</dt>
                            <dd className="font-medium capitalize">{employee.employment_type ?? "—"}</dd>
                        </div>
                        <div>
                            <dt className="text-xs text-gray-500 mb-1">Status</dt>
                            <dd><Badge color={employee.status === "active" ? "green" : "red"}>{employee.status}</Badge></dd>
                        </div>
                        {employee.bank_account_number && (
                            <div>
                                <dt className="text-xs text-gray-500">Bank Account</dt>
                                <dd className="font-medium">{employee.bank_account_number}</dd>
                            </div>
                        )}
                        {employee.address && (
                            <div className="col-span-2">
                                <dt className="text-xs text-gray-500">Address</dt>
                                <dd className="text-gray-700">{employee.address}</dd>
                            </div>
                        )}
                    </dl>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100">
                    <div className="px-6 py-4 border-b border-gray-100">
                        <h3 className="text-sm font-semibold text-gray-700">Recent Payroll</h3>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {employee.payroll_records?.length ? employee.payroll_records.map((pr) => (
                                    <tr key={pr.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 font-medium">{pr.payroll?.period_start} → {pr.payroll?.period_end}</td>
                                        <td className="px-4 py-3">৳{Number(pr.gross_salary ?? 0).toLocaleString()}</td>
                                        <td className="px-4 py-3">৳{Number(pr.total_deduction ?? 0).toLocaleString()}</td>
                                        <td className="px-4 py-3 font-medium">৳{Number(pr.net_salary ?? 0).toLocaleString()}</td>
                                        <td className="px-4 py-3"><Badge color={pr.status === "paid" ? "green" : "yellow"}>{pr.status}</Badge></td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">No payroll records</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
