...
export default function UserForm({ editUser, roles }) {
    ...
    const { data, setData, post, put, processing, errors } = useForm({
        ...
        role:      editUser?.role ?? (roles?.[0]?.name ?? "admin"),
    ...
    const submit = (e) => {
        e.preventDefault();
        isEdit ? put(route("settings.users.update", editUser.id)) : post(route("settings.users.store"));
    };

    return (
        <AppLayout title={isEdit ? "Edit User" : "Add User"}>
            <Head title={isEdit ? "Edit User" : "Add User"} />
            <PageHeader
                title={isEdit ? "Edit User" : "Add User"}
                subtitle={isEdit ? `Editing: ${editUser.name}` : "Create a new team member"}
                actions={
                    <Link href={route("settings.users.index")} className="flex items-center gap-2 text-slate-600 text-sm font-medium">
            ...
            <form onSubmit={submit} className="max-w-2xl space-y-6">
                <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <Field label={isEdit ? "New Password (leave blank to keep)" : "Password *"} error={errors.password}>
                            <Input type="password" value={data.password} onChange={e => setData("password", e.target.value)} required={!isEdit} autoComplete="new-password" />
                        </Field>
                        <Field label="Confirm Password" error={errors.password_confirmation}>
                            <Input type="password" value={data.password_confirmation} onChange={e => setData("password_confirmation", e.target.value)} required={!isEdit} autoComplete="new-password" />
                        </Field>
                ...
                <div className="flex justify-end gap-3 pb-8">
                    <Link href={route("settings.users.index")} className="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">Cancel</Link>
