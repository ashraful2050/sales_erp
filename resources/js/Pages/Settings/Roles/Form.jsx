...
export default function RoleForm({
    ...
    const [expandedMods, setExpandedMods] = useState(() => {
        ...
        return Object.keys(featureLabels ?? {}).reduce(
            (acc, m) => ({ ...acc, [m]: true }),
            {},
        );
    });

    // True when this permission key can be toggled by the current user.
    // In admin mode, only keys in adminPermissions are allowed.
    const isAllowed = (key) =>
        !isAdminMode || (adminPermissions ?? []).includes(key);
    ...
    const toggleModule = (mod) => {
        ...
        const allowedKeys = actions.map((a) => `${mod}.${a}`).filter(isAllowed);
        if (!allowedKeys.length) return;
        const allOn = allowedKeys.every((k) => data.permissions[k]);
    ...
    const toggleAction = (action) => {
        const keys = Object.keys(modules)
            .map((mod) => `${mod}.${action}`)
            .filter((k) => k in data.permissions && isAllowed(k));
    ...
    const toggleFeatureModule = (mod) => {
        ...
        const allowedKeys = feats.map((f) => `${mod}.${f}`).filter(isAllowed);
        if (!allowedKeys.length) return;
        const allOn = allowedKeys.every((k) => data.permissions[k]);
        const updated = { ...data.permissions };
        allowedKeys.forEach((k) => {
    ...
    const toggleAllFeatures = (on) => {
        ...
        Object.entries(featureLabels ?? {}).forEach(([mod, feats]) => {
            Object.keys(feats).forEach((f) => {
                const key = `${mod}.${f}`;
                if (!on || isAllowed(key)) updated[key] = on;
            });
        });
        setData("permissions", updated);
    };
    ...
    const submit = (e) => {
        e.preventDefault();
        isEdit
            ? put(route("settings.roles.update", editRole.id))
    ...
    // Count helpers
    const modulePermCount = Object.entries(data.permissions).filter(
        ([k, v]) => {
            const mod = k.split(".")[0];
    ...
    const featurePermCount = Object.entries(data.permissions).filter(
        ([k, v]) => {
            const [mod, feat] = k.split(".");
            return v && featureLabels[mod]?.[feat] !== undefined;
        },
    ).length;
    const totalFeatureCount = Object.values(featureLabels ?? {}).reduce(
        (s, f) => s + Object.keys(f).length,
        0,
    );
    const totalModuleCount = Object.values(modules).reduce(
        (s, a) => s + a.length,
        0,
    );
                ...
                <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                    <div className="flex border-b border-slate-200">
                        {[
                            ...
                            >
                                <p
                                    className={`text-sm font-semibold ${activeTab === tab.id ? "text-blue-700" : "text-slate-600"}`}
                                >
                                    {tab.label}
                                </p>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {Object.entries(modules).map(
                                        ([mod, actions]) => {
                                            ...
                                            const modAllOn = actions.every(
                                                (a) =>
                                                    data.permissions[
                                                        `${mod}.${a}`
                                                    ],
                                            );
                                            const modSomeOn = actions.some(
                                                (a) =>
                                                    data.permissions[
                                                        `${mod}.${a}`
                                                    ],
                                                                ...
                                                                >
                                                                    {exists ? (
                                                                        <button
                                                                            ...
                                                                            title={
                                                                                locked
                                                                                    ? "Your admin account does not have this permission"
                                                                                    : undefined
                                                                            }
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            {Object.entries(featureLabels ?? {}).map(
                                ([mod, feats]) => {
                                    ...
                                    const enabled = featKeys.filter(
                                        (f) => data.permissions[`${mod}.${f}`],
                                    ).length;
                                    const allOn = enabled === featKeys.length;
                                    const expanded =
                                        expandedMods[mod] !== false;
                                    ...
                                    return (
                                        ...
                                        >
                                            {/* Module header */}
                                            <div
                                                className={`${color.bg} px-4 py-3 flex items-center justify-between`}
                                            >
