...
export default function GoodsReceiptShow({ grn }) {
    const totalCost = grn.items?.reduce((s, i) => s + Number(i.total_cost ?? 0), 0) ?? 0;
        ...
        <AppLayout title={`GRN — ${grn.grn_number}`}>
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                    <div className="px-6 py-4 border-b"><h3 className="text-sm font-semibold">Items Received</h3></div>
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
                            <tr>
                                {["Product", "Qty Received", "Unit Cost", "Total Cost"].map(h => (
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {grn.items?.map((item, i) => (
                                <tr key={i} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3">{Number(item.quantity_received).toLocaleString()}</td>
                                    <td className="px-4 py-3">৳{Number(item.unit_cost).toLocaleString()}</td>
                                    <td className="px-4 py-3 font-semibold">৳{Number(item.total_cost).toLocaleString()}</td>
                                </tr>
                            ))}
                        </tbody>
                        <tfoot className="bg-gray-50 border-t">
                            <tr>
                                ...
                                <td className="px-4 py-3 font-bold text-blue-700">৳{totalCost.toLocaleString()}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
