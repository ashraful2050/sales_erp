...
export default function DebitNoteForm({ debitNote, vendors, accounts }) {
    ...
    const subtotal = data.items.reduce(
        (s, it) =>
            s + parseFloat(it.unit_price || 0) * parseFloat(it.quantity || 0),
        0,
    );
    const taxTotal = data.items.reduce(
        (s, it) => s + parseFloat(it.tax_amount || 0),
        0,
    );
    ...
    const submit = (e) => {
        e.preventDefault();
        isEdit
            ? put(route("purchase.debit-notes.update", debitNote.id))
            : post(route("purchase.debit-notes.store"));
    };
    ...
    return (
        <AppLayout title={isEdit ? t("Edit Debit Note") : t("New Debit Note")}>
            <Head title={isEdit ? t("Edit Debit Note") : t("New Debit Note")} />
            <PageHeader
                title={
                    isEdit
                        ? `${t("Edit")}: ${debitNote.po_number}`
                        : t("New Debit Note")
                }
                subtitle={t("Purchase return")}
                actions={
                    <Link
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            {errors.vendor_id && (
                                <p className={errCls}>{errors.vendor_id}</p>
                            )}
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">
                            ...
                            />
                            {errors.po_date && (
                                <p className={errCls}>{errors.po_date}</p>
                            )}
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className={inputCls}
                                placeholder={t("Original PO / Invoice ref")}
                            />
                        </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-slate-50 border-b border-slate-200">
                                <tr>
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase w-1/3">
                                        {t("Description")}
                                    </th>
