...
export default function VendorForm({ vendor }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        isEdit
            ? put(route("purchase.vendors.update", vendor.id))
            : post(route("purchase.vendors.store"));
    };

    return (
        <AppLayout title={isEdit ? "Edit Vendor" : "New Vendor"}>
            <Head title={isEdit ? "Edit Vendor" : "New Vendor"} />
            <PageHeader
            ...
            <form onSubmit={submit} className="max-w-4xl space-y-6">
                <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Field label="Vendor Name (English) *" error={errors.name}>
                            <Input value={data.name} onChange={e => setData("name", e.target.value)} required />
                        </Field>
                        <Field label="Vendor Name (Bangla)" error={errors.name_bn}>
                ...
                <div className="flex justify-end gap-3 pb-8">
                    <Link href={route("purchase.vendors.index")} className="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">Cancel</Link>
                    <button type="submit" disabled={processing}
                        ...
                        <Save size={16} /> {isEdit ? "Update Vendor" : "Create Vendor"}
                    </button>
                </div>
            </form>
        </AppLayout>
