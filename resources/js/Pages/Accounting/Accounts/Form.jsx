...
export default function AccountForm({ account, groups }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        isEdit
            ? put(route("accounting.accounts.update", account.id))
            : post(route("accounting.accounts.store"));
    };

    return (
        <AppLayout title={isEdit ? t("Edit Account") : t("New Account")}>
            <Head title={isEdit ? t("Edit Account") : t("New Account")} />
            <PageHeader
                title={isEdit ? t("Edit Account") : t("New Account")}
                subtitle={
                    isEdit
                        ? `${t("Editing")}: ${account.name}`
                        : t("Add a new chart of accounts entry")
                }
                actions={
                    <Link
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        >
                            <Select
                                ...
                                onChange={(e) =>
                                    setData("account_group_id", e.target.value)
                                }
                            >
