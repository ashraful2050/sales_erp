...
export default function Show({ entry }) {
    ...
    const totalDebits =
        entry.lines?.reduce((s, l) => s + Number(l.debit ?? 0), 0) ?? 0;
    const totalCredits =
        entry.lines?.reduce((s, l) => s + Number(l.credit ?? 0), 0) ?? 0;
    ...
    return (
        <AppLayout>
            ...
            <div className="p-6 space-y-6">
                ...
                {/* Balance indicator */}
                <div
                    className={`rounded-xl px-4 py-3 text-sm font-medium ${isBalanced ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
                >
                    {isBalanced
                        ? "✓ Entry is balanced"
