...
export default function BudgetForm({ budget, lines: initLines, fiscalYears, costCenters, accounts }) {
    ...
    const { data, setData, post, put, processing, errors } = useForm({
        ...
        lines: initLines?.length > 0
            ? initLines.map(l => ({ account_id: String(l.account_id), month: l.month ?? "", budgeted_amount: l.budgeted_amount }))
            : [{ account_id: "", month: "", budgeted_amount: "" }],
    });

    const addLine = () => setData("lines", [...data.lines, { account_id: "", month: "", budgeted_amount: "" }]);
    ...
    const totalBudgeted = data.lines.reduce((s, l) => s + (parseFloat(l.budgeted_amount) || 0), 0);

    return (
        <AppLayout title={isEdit ? "Edit Budget" : "New Budget"}>
            <Head title={isEdit ? "Edit Budget" : "New Budget"} />
            <PageHeader title={isEdit ? "Edit Budget" : "New Budget"}
                breadcrumbs={[{ label: "Budgets", href: route("accounting.budgets.index") }, { label: isEdit ? "Edit" : "New" }]}
            />
            <form onSubmit={submit} className="space-y-6">
                {/* Header */}
                <div className="bg-white rounded-xl border border-slate-200 p-6">
                    <h3 className="font-semibold text-slate-700 mb-4">Budget Details</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-slate-700 mb-1">Budget Name <span className="text-red-500">*</span></label>
                            <input value={data.name} onChange={e => setData("name", e.target.value)}
                                className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" placeholder="e.g. Annual Budget 2025" />
                        ...
                        <div>
                            ...
                            <select value={data.cost_center_id} onChange={e => setData("cost_center_id", e.target.value)}
                                ...
                                {costCenters.map(cc => <option key={cc.id} value={cc.id}>{cc.name}{cc.code ? ` (${cc.code})` : ""}</option>)}
                            </select>
                        </div>
                        <div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {data.lines.map((line, i) => (
                                    <tr key={i} className="hover:bg-slate-50">
                                        ...
                                        {data.period === "monthly" && (
                                            <td className="px-4 py-2">
                                                <select value={line.month} onChange={e => updateLine(i, "month", e.target.value)}
                                        ...
                                        <td className="px-4 py-2">
                                            <input type="number" value={line.budgeted_amount} onChange={e => updateLine(i, "budgeted_amount", e.target.value)}
                                                className="border border-slate-300 rounded-lg px-2 py-1.5 text-sm text-right w-full"
                                                placeholder="0.00" min="0" step="0.01" />
                                        </td>
                                        <td className="px-4 py-2 text-center">
                                            <button type="button" onClick={() => removeLine(i)} className="p-1 text-slate-300 hover:text-red-500 rounded"><Trash2 size={14} /></button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                            <tfoot className="bg-slate-50 border-t-2 border-slate-200">
                                <tr>
                                    <td className="px-4 py-3 font-semibold text-slate-700" colSpan={data.period === "monthly" ? 2 : 1}>Total Budget</td>
                                    <td className="px-4 py-3 text-right font-semibold text-slate-800">
                                        {totalBudgeted.toLocaleString("en-BD", { minimumFractionDigits: 2 })} BDT
                                    </td>
                                    <td></td>
                                </tr>
                            </tfoot>
                ...
                <div className="flex items-center gap-3">
                    <button type="submit" disabled={processing}
                        ...
                        {processing ? "Saving..." : (isEdit ? "Update Budget" : "Create Budget")}
                    </button>
                    <Link href={route("accounting.budgets.index")} className="text-sm text-slate-500 hover:text-slate-700">Cancel</Link>
                </div>
            </form>
        </AppLayout>
    );
