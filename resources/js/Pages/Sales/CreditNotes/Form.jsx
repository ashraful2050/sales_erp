...
export default function CreditNoteForm({ note, customers, products, taxRates }) {
    ...
    const { data, setData, post, put, processing, errors } = useForm({
        ...
        invoice_date: note?.invoice_date ?? new Date().toISOString().slice(0, 10),
        notes: note?.notes ?? "",
        items: note?.items?.length
            ? note.items.map(i => ({ product_id: i.product_id ?? "", description: i.description, quantity: i.quantity, unit_price: i.unit_price, tax_rate: i.tax_rate ?? 0, tax_amount: i.tax_amount ?? 0, total: i.total }))
            : [emptyLine()],
    });
    ...
    const updateLine = (idx, field, val) => {
        ...
        const { quantity, unit_price, tax_rate } = items[idx];
        const baseTotal = Number(quantity || 0) * Number(unit_price || 0);
        items[idx].tax_amount = baseTotal * (Number(tax_rate || 0) / 100);
        items[idx].total = baseTotal + items[idx].tax_amount;
        setData("items", items);
    };
    ...
    const submit = (e) => {
        e.preventDefault();
        const payload = { ...data };
        if (isEdit) put(route("sales.credit-notes.update", note.id), payload);
        else post(route("sales.credit-notes.store"), payload);
    };
        ...
        <AppLayout title={isEdit ? "Edit Credit Note" : "New Credit Note"}>
            ...
            <form onSubmit={submit} className="space-y-6">
                <div className="bg-white rounded-xl border border-slate-200 p-6">
                    <h3 className="font-semibold text-slate-700 mb-4">Credit Note Details</h3>
                                    ...
                                    <tr key={i} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-3 py-2">
                                            <input type="number" value={line.quantity} onChange={e => updateLine(i, "quantity", e.target.value)}
                                                className="border border-slate-200 rounded-lg px-2 py-1.5 text-xs text-right w-full" min="0.0001" step="0.0001" />
                                        </td>
                                        <td className="px-3 py-2">
                                            <input type="number" value={line.unit_price} onChange={e => updateLine(i, "unit_price", e.target.value)}
                                                className="border border-slate-200 rounded-lg px-2 py-1.5 text-xs text-right w-full" min="0" step="0.01" />
                                        </td>
                                        <td className="px-3 py-2">
                                            <input type="number" value={line.tax_rate} onChange={e => updateLine(i, "tax_rate", e.target.value)}
                                                className="border border-slate-200 rounded-lg px-2 py-1.5 text-xs text-right w-full" min="0" max="100" step="0.01" />
                                        </td>
                ...
                <div className="flex items-center gap-3">
                    <button type="submit" disabled={processing}
                        ...
                        {processing ? "Saving…" : (isEdit ? "Update Credit Note" : "Create Credit Note")}
                    </button>
                    <Link href={route("sales.credit-notes.index")} className="text-sm text-slate-500 hover:text-slate-700">Cancel</Link>
                </div>
            </form>
        </AppLayout>
    );
