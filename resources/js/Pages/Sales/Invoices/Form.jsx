...
const Input = ({ error, className = "", ...props }) => (
    <input {...props} className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${error ? "border-red-400" : "border-slate-200"} ${className}`} />
);
...
export default function InvoiceForm({ invoice, customers, products, taxRates }) {
    ...
    const { data, setData, post, put, processing, errors } = useForm({
        ...
        items: invoice?.items?.length
            ? invoice.items.map(i => ({ ...i, line_total: Number(i.quantity) * Number(i.unit_price) * (1 - Number(i.discount_pct) / 100) }))
            : [emptyLine()],
    });
    ...
    const updateLine = (idx, field, val) => {
        ...
        items[idx].line_total = Number(quantity) * Number(unit_price || 0) * (1 - Number(discount_pct || 0) / 100);
        setData("items", items);
    };
    ...
    const selectProduct = (idx, productId) => {
        ...
        if (p) {
            const items = [...data.items];
            items[idx] = { ...items[idx], product_id: productId, description: p.name, unit_price: p.sale_price, tax_rate_id: p.tax_rate_id ?? "" };
            items[idx].line_total = Number(items[idx].quantity) * Number(p.sale_price) * (1 - Number(items[idx].discount_pct || 0) / 100);
            setData("items", items);
        } else {
    ...
    const subtotal = data.items.reduce((s, i) => s + Number(i.line_total || 0), 0);
    const taxTotal = data.items.reduce((s, i) => {
        const rate = taxRates?.find(t => String(t.id) === String(i.tax_rate_id));
        return s + (rate ? Number(i.line_total) * Number(rate.rate) / 100 : 0);
    }, 0);
        ...
        <AppLayout title={isEdit ? "Edit Invoice" : "New Invoice"}>
            ...
            <form onSubmit={submit} className="space-y-6">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {data.items.map((item, idx) => (
                                    <tr key={idx}>
                                        ...
                                        <td className="px-4 py-2">
                                            <Input type="number" step="any" min="0" value={item.quantity} onChange={e => updateLine(idx, "quantity", e.target.value)} className="text-right" error={errors[`items.${idx}.quantity`]} />
                                            {errors[`items.${idx}.quantity`] && <p className="text-xs text-red-500 mt-1">{errors[`items.${idx}.quantity`]}</p>}
                                        </td>
                                        <td className="px-4 py-2">
                                            <Input type="number" step="any" min="0" value={item.unit_price} onChange={e => updateLine(idx, "unit_price", e.target.value)} className="text-right" />
                                        </td>
                                        <td className="px-4 py-2">
                                            <Input type="number" step="any" min="0" max="100" value={item.discount_pct} onChange={e => updateLine(idx, "discount_pct", e.target.value)} className="text-right" />
                                        </td>
                                        <td className="px-4 py-2">
                                        ...
                                        <td className="px-4 py-2 text-right font-mono text-slate-700">
                                            ৳{Number(item.line_total || 0).toLocaleString("en-BD", { minimumFractionDigits: 2 })}
                                        </td>
                ...
                <div className="flex justify-end gap-3 pb-8">
                    <Link href={route("sales.invoices.index")} className="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">Cancel</Link>
                    <button type="submit" disabled={processing}
                        ...
                        <Save size={16} /> {isEdit ? "Update Invoice" : "Save Invoice"}
                    </button>
                </div>
            </form>
        </AppLayout>
