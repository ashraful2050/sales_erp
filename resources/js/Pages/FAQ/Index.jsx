...
export default function FaqIndex({ categories, faqs, filters }) {
    ...
    const doSearch = (s, cat) => {
        router.get(
            route("faq.index"),
            { search: s || undefined, category: cat || undefined },
        ...
        <AppLayout title={t("Knowledge Base")}>
            ...
            <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-8 mb-6 text-white relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-2xl">
                    ...
                    <p className="text-blue-200 mb-5">
                        {t("Search our knowledge base or browse by category")}
                    </p>
                    <form onSubmit={handleSearch} className="flex gap-2">
                        ...
                        <button
                            type="submit"
                            className="bg-white/20 hover:bg-white/30 text-white px-5 py-2.5 rounded-xl text-sm font-semibold border border-white/20"
                        >
                            {t("Search")}
                        </button>
                    </form>
            ...
            {faqs?.length === 0 ? (
                ...
                <div className="space-y-2">
                    {(faqs ?? []).map((faq) => {
                        const catColor =
                            COLOR_MAP[faq.category?.color] ?? COLOR_MAP.slate;
                        return (
                            <Link
                                key={faq.id}
                            ...
                            >
                                <div className="flex items-start gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        <p className="font-semibold text-slate-800 group-hover:text-blue-700 transition-colors leading-snug">
                                            {faq.question}
                                        </p>
                                ...
                                <ChevronRight
                                    size={16}
                                    className="text-slate-300 group-hover:text-blue-500 shrink-0 ml-4 transition-colors"
                                />
                            </Link>
                        );
                    })}
