...
const TYPE_CONFIG = {
    in:          { label: "Stock In",    icon: PackagePlus,  color: "bg-green-50 border-green-200",  iconColor: "text-green-600", desc: "Receive products into warehouse (purchase, return, opening stock)" },
    out:         { label: "Stock Out",   icon: PackageMinus, color: "bg-red-50 border-red-200",      iconColor: "text-red-500",   desc: "Remove products from warehouse (sales, damage, write-off)" },
    adjustment:  { label: "Adjustment",  icon: RefreshCw,    color: "bg-amber-50 border-amber-200",  iconColor: "text-amber-600", desc: "Correct stock quantity (physical count difference)" },
};
        ...
        <AppLayout title={t("New Stock Entry")}>
            ...
            <form onSubmit={submit} className="max-w-2xl space-y-6">
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-6">
                    <h3 className="text-sm font-semibold text-slate-700 mb-3">Movement Type</h3>
                    <div className="grid grid-cols-3 gap-3">
                        {Object.entries(TYPE_CONFIG).map(([type, c]) => {
                            const Ic = c.icon;
                            const active = data.movement_type === type;
                            return (
                                <button key={type} type="button" onClick={() => setData("movement_type", type)}
                    ...
                    <p className={`mt-3 text-xs px-3 py-2 rounded-lg border ${cfg.color} ${cfg.iconColor}`}>
                        <Icon size={12} className="inline mr-1 mb-0.5" />{cfg.desc}
                    </p>
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2">
                            ...
                            {errors.product_id && <p className={errCls}>{errors.product_id}</p>}
                        </div>

                        {/* Warehouse */}
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Warehouse <span className="text-red-500">*</span></label>
                            <select value={data.warehouse_id} onChange={e => setData("warehouse_id", e.target.value)} className={inputCls}>
                                <option value="">-- Select Warehouse --</option>
                                {warehouses.map(w => (
                                    <option key={w.id} value={w.id}>{w.name}</option>
                                ))}
                            </select>
                            {errors.warehouse_id && <p className={errCls}>{errors.warehouse_id}</p>}
                        </div>

                        {/* Date */}
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Date <span className="text-red-500">*</span></label>
                            <input type="date" value={data.movement_date} onChange={e => setData("movement_date", e.target.value)} className={inputCls} />
                            {errors.movement_date && <p className={errCls}>{errors.movement_date}</p>}
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">
                                ...
                                {data.movement_type === "out" && <span className="text-xs text-red-400 ml-1">(units to remove)</span>}
                                {data.movement_type === "adjustment" && <span className="text-xs text-amber-500 ml-1">(+ to add, use negative to reduce)</span>}
                            </label>
                            <input type="number" step="0.0001" value={data.quantity} onChange={e => setData("quantity", e.target.value)}
                    ...
                    {selectedProduct && (
                        <div className="bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm grid grid-cols-3 gap-4">
                            <div><span className="text-slate-500">Cost Price:</span> <span className="font-medium ml-1">৳{Number(selectedProduct.cost_price || 0).toLocaleString()}</span></div>
                            <div><span className="text-slate-500">Sale Price:</span> <span className="font-medium ml-1">৳{Number(selectedProduct.sale_price || 0).toLocaleString()}</span></div>
                            <div><span className="text-slate-500">Code:</span> <span className="font-medium ml-1">{selectedProduct.code || "—"}</span></div>
                        </div>
                    )}
                </div>

                <div className="flex justify-end gap-3">
                    <Link href={route("inventory.stock-movements.index")} className="px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50">
                        Cancel
                    </Link>
                    <button type="submit" disabled={processing}
                        ...
                        <Save size={16} /> {processing ? "Saving..." : `Record ${TYPE_CONFIG[data.movement_type].label}`}
                    </button>
                </div>
            </form>
        </AppLayout>
