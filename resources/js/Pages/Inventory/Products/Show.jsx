...
export default function Show({ product }) {
    const totalStock = product.stocks?.reduce((s, st) => s + Number(st.quantity ?? 0), 0) ?? 0;
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <dl className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                        <div><dt className="text-xs text-gray-500">Category</dt><dd className="font-medium">{product.category?.name ?? "—"}</dd></div>
                        <div><dt className="text-xs text-gray-500">Unit</dt><dd className="font-medium">{product.unit?.name ?? "—"}</dd></div>
                        <div><dt className="text-xs text-gray-500">Tax Rate</dt><dd className="font-medium">{product.taxRate ? `${product.taxRate.rate}%` : "—"}</dd></div>
                        <div><dt className="text-xs text-gray-500">Reorder Level</dt><dd className="font-medium">{product.reorder_level ?? "—"}</dd></div>
                        <div>
                            <dt className="text-xs text-gray-500 mb-1">Status</dt>
                            <dd><Badge color={product.is_active ? "green" : "red"}>{product.is_active ? "Active" : "Inactive"}</Badge></dd>
                        </div>
                    </dl>
                    {product.description && (
                ...
                {product.stocks?.length > 0 && (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100">
                        <div className="px-6 py-4 border-b border-gray-100">
                            <h3 className="text-sm font-semibold text-gray-700">Stock by Warehouse</h3>
                        </div>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {product.stocks.map((stock) => (
                                    <tr key={stock.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 font-medium">{stock.warehouse?.name ?? "—"}</td>
                                        <td className="px-4 py-3">{stock.quantity} {product.unit?.name ?? ""}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
                                <tr>
                                    {["Type", "Quantity", "Reference", "Date", "Notes"].map((h) => (
                                        <th key={h} className="px-4 py-3 text-left">{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {product.stock_movements?.length ? product.stock_movements.map((m) => (
                                    <tr key={m.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex items-center gap-1 text-xs font-medium ${m.type === "in" ? "text-green-700" : "text-red-700"}`}>
                                    ...
                                    <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">No stock movements</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
