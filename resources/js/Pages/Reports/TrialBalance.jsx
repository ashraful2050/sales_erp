...
export default function TrialBalance({ report, filters }) {
    ...
    const totalDebit =
        report?.reduce((s, r) => s + Number(r.debit || 0), 0) ?? 0;
    const totalCredit =
        report?.reduce((s, r) => s + Number(r.credit || 0), 0) ?? 0;
    ...
    return (
        <AppLayout title={t("Trial Balance")}>
            ...
            <PageHeader
                ...
                subtitle={t(
                    "View all account balances as debit and credit totals",
                )}
                actions={
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-slate-50 border-b border-slate-200">
                                <tr>
                                    <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase w-24">
                                        {t("Code")}
                                    </th>
                                ...
                                {report.map((row, i) => (
                                    <tr key={i} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-6 py-3 text-right font-mono text-slate-700">
                                            {Number(row.debit) > 0
                                                ? `৳${Number(row.debit).toLocaleString("en-BD", { minimumFractionDigits: 2 })}`
                                                : "—"}
                                        </td>
                                        <td className="px-6 py-3 text-right font-mono text-slate-700">
                                            {Number(row.credit) > 0
                                                ? `৳${Number(row.credit).toLocaleString("en-BD", { minimumFractionDigits: 2 })}`
                                                : "—"}
                                        </td>
                                    </tr>
                                ))}
                            ...
                            <tfoot className="bg-slate-50 border-t-2 border-slate-300">
                                <tr>
                                    <td
                                        colSpan={3}
                                        className="px-6 py-3 text-sm font-bold text-slate-700"
                                    >
                                        Total
                                    </td>
                                ...
                                <tr>
                                    <td
                                        colSpan={5}
                                        className="px-6 py-2 text-right text-xs"
                                    >
