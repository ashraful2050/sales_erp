...
function BSSection({ title, groups }) {
    ...
    return (
        <div className="flex-1">
            <div className="px-4 py-2 bg-slate-700 text-white text-xs font-bold uppercase tracking-wider">
                {title}
            </div>
            {groups?.map((g, gi) => {
                const groupTotal =
                ...
                return (
                    <div key={gi}>
                        <div className="px-4 py-1.5 bg-slate-100 text-xs font-semibold text-slate-600 uppercase">
                            {g.name}
                        </div>
                        {g.accounts?.map((a, ai) => (
                            <div
                                key={ai}
                                className="flex justify-between px-6 py-2 border-b border-slate-50 text-sm hover:bg-slate-50"
                            >
                                <span className="text-slate-600">{a.name}</span>
                                <span className="font-mono text-slate-700">
                                    ৳
                                    {Number(a.balance).toLocaleString("en-BD", {
                        ...
                        <div className="flex justify-between px-4 py-2 border-b border-slate-200 bg-slate-50 text-xs font-semibold">
                            ...
                            <span className="font-mono">
                                ৳
                                {groupTotal.toLocaleString("en-BD", {
                                    minimumFractionDigits: 2,
                                })}
                            </span>
                        </div>
            ...
            <div className="flex justify-between px-4 py-3 bg-slate-200 text-sm font-bold">
                ...
                <span className="font-mono">
                    ৳
                    {sectionTotal.toLocaleString("en-BD", {
                        minimumFractionDigits: 2,
                    })}
                </span>
            </div>
...
export default function BalanceSheet({ report, filters }) {
    ...
    const [asOf, setAsOf] = useState(
        filters?.as_of ?? new Date().toISOString().slice(0, 10),
    );
    const run = () =>
        router.get(
