...
export default function PayrollSummary({ month }) {
    ...
    const handleFilter = () => {
        router.get(route("reports.payroll-summary"), { month: selectedMonth }, { preserveState: true });
    };
    ...
    const monthName = new Date(`${year}-${mon}-01`).toLocaleString("default", { month: "long" });
        ...
        <AppLayout>
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        { label: "Employees", value: "—", icon: "👥", bg: "bg-blue-50 text-blue-700" },
                        { label: "Gross Payroll", value: "৳0", icon: "💰", bg: "bg-indigo-50 text-indigo-700" },
                        { label: "Deductions", value: "৳0", icon: "📉", bg: "bg-red-50 text-red-700" },
                        { label: "Net Payroll", value: "৳0", icon: "✅", bg: "bg-green-50 text-green-700" },
                    ].map((c) => (
                        <div key={c.label} className={`rounded-xl p-4 ${c.bg}`}>
                            <p className="text-lg">{c.icon}</p>
                            <p className="text-xs font-medium uppercase tracking-wide opacity-70 mt-1">{c.label}</p>
                        ...
                        <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
                            <tr>
                                {["Description", "Amount"].map((h) => (
                                    <th key={h} className={`px-6 py-3 ${h === "Amount" ? "text-right" : "text-left"}`}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                            ...
                            {summaryRows.map((row) => (
                                <tr key={row.label} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-3 text-right font-medium">
                                        {row.currency ? `৳${row.value.toLocaleString()}` : row.value || "—"}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div className="px-6 py-12 text-center text-gray-400 text-sm">
                        <p>No payroll data found for {monthName} {year}.</p>
                        <p className="mt-1 text-xs">Process payroll for this period to see the department breakdown.</p>
                    </div>
                </div>

                <p className="text-xs text-gray-400 text-center">
                    This report summarises all processed payroll records for the selected month.
                </p>
            </div>
        </AppLayout>
    );
