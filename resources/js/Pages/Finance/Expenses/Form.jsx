...
export default function ExpenseForm({ expense, categories, bankAccounts }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        isEdit ? put(route("finance.expenses.update", expense.id)) : post(route("finance.expenses.store"));
    };

    return (
        <AppLayout title={isEdit ? "Edit Expense" : "New Expense"}>
            ...
            <form onSubmit={submit} className="max-w-2xl space-y-6">
                <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <Field label="Expense Title *" error={errors.title}>
                            <Input value={data.title} onChange={e => setData("title", e.target.value)} placeholder="e.g. Office Rent, Utility Bill" required className="md:col-span-2" />
                        </Field>
                        <Field label="Amount (৳) *" error={errors.amount}>
                            <Input type="number" step="any" min="0.01" value={data.amount} onChange={e => setData("amount", e.target.value)} required />
                        </Field>
                ...
                <div className="flex justify-end gap-3 pb-8">
                    <Link href={route("finance.expenses.index")} className="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">Cancel</Link>
                    <button type="submit" disabled={processing}
                        ...
                        <Save size={16} /> {isEdit ? "Update Expense" : "Save Expense"}
                    </button>
                </div>
            </form>
        </AppLayout>
